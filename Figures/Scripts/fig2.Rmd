...
df_final= df %>% mutate(n=ifelse(is.na(n), 0, n)) %>% 
  ...
  mutate(strain=factor(strain, levels=c("12w Y", "26w NE", "26w IE")))
...
t<- tab_add_title(t,text = "26w IE: cell numbers", face = "bold", size = 8)   
t
```
...
p3<-subset(sobj, strain=="26w IE" & timepoint=="2days" & compartments=="Hematoma") %>%
...
p4 <- subset(sobj, strain=="26w IE" & timepoint=="5days" & compartments=="Hematoma" ) %>%
  DimPlot(label=F, group.by="CellTypes",pt.size = .2, cells.highlight= list(g1_untreat),sizes.highlight=.2)+
  theme(text = element_text(family = "Arial"),
        plot.title = element_text(size = 8,face="bold"),
        axis.title = element_text(size = 8), 
...
sF1<-ggplot(df_final, aes(x = compartments, y = p)) +
  ...
  facet_wrap(~strain+timepoint, ncol = 8, scales = "free_x")+
  theme(axis.title = element_text(size = 8), 
        axis.text = element_text(size = 8, family = "Arial"),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
  ...
  labs(title ="Day 2 / Day5 all bone compartments comparisons \nB cells",x = c(color =''), y = 'Percentage of cells') 

sF1

```
supplement figure mice age comparison

```{r sF2, fig.width = 15, fig.height = 3}


my_comparisons <- list(c("12w Y", "26w NE"), c("12w Y", "26w IE"),  c("26w NE", "26w IE"))
...
sF2<-ggplot(df_final, aes(x = strain, y = p)) +
  geom_point(aes(color = strain), position=position_jitterdodge(jitter.width = .2), shape=1, size=2) +
  coord_cartesian(ylim=c(0,30))+
  stat_compare_means(comparisons = my_comparisons,label="p.signif",
                     method = "t.test", hide.ns = FALSE,size=2,paired = F) +
  facet_wrap(~compartments+timepoint, ncol = 8, scales = "free_x") +
  theme(axis.title = element_text(size = 8), 
        axis.text = element_text(size = 8, family = "Arial"),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
        ...
        strip.text.y = element_text(size = 8),
        plot.title = element_text(size = 8,face="bold", hjust = 0.5,))+
  scale_color_manual(name= "Mice age",values = c("dodgerblue4","darkgreen","darkred")) + 
  labs(title ="Day 2 / Day 5 all mice age comparisons \nB Cells",x = c(color =''), y = 'Percentage of cells') 
